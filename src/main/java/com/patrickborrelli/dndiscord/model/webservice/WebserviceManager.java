...
/**
 * Singleton instance responsible for adding, modifying, and deleting back end
 * data.
 * 
...
public class WebserviceManager {
	...
	private static final String FORMULA_BY_NAME_URL;
	...
	public CharacterSheet addUserCharacter(DiscordUser user, CharacterSheet character) {
		...
		if (character.getActions() != null)
			addActionsToCharacter(character);
		if (character.getAttacks() != null)
			addAttacksToCharacter(character);
		if (character.getCharacterClasses() != null)
	...
	public String setActiveCharacter(String userId, String characterId) {
		...
		buf.append(ACTIVE_URL).append("/").append(userId).append("/").append(characterId);
	...
	/**
	 * Retrieves a user saved formula by name. Returns a string representation of
	 * the roll formula or null if none exists.
	 * 
	 * @param user a DiscordUser
	...
	public String getUserFormula(DiscordUser user, String name) {
		StringBuilder url = new StringBuilder().append(FORMULA_BY_NAME_URL).append("/").append(user.getId()).append("/")
				.append(name);
		...
		try {
			...
			if (responseCode != HttpURLConnection.HTTP_OK) {
				...
				if (response.toString().equalsIgnoreCase("null") || response.toString().equalsIgnoreCase("[]")) {
					...
					formula = FORMULA_READER.readValue(response.toString());
				}
	...
	/**
	 * Retrieves a list of all saved roll Formulas belonging to the provided user.
	 * 
	 * @param user a DiscordUser
	 * @return a List of Formula objects containing all saved rolls for the provided
	 *         user.
	 */
	...
	/**
	 * Retrieves a comma-delimited string containing all characters currently
	...
	public DiscordUser getUser(String discordId) {
		StringBuilder buf = new StringBuilder();
		buf.append(USER_URL + "/").append(discordId);
	 ...
	 */
	public DiscordUser createUser(String id, String username, String discriminator, String avatar, boolean bot) {
