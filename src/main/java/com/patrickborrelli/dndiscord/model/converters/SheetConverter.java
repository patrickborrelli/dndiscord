package com.patrickborrelli.dndiscord.model.converters;
...
public class SheetConverter {
	...
	public CharacterSheet convertFormat(SheetSourceType type, DndBeyondSheet character) {
		...
		if (type == SheetSourceType.BEYOND) {
			...
			if (align != null)
				sheet.setAlignment(align.getStringValue());
			if (life != null)
				sheet.setLifestyle(life.getStringValue());
	...
	private Action convertAction(com.patrickborrelli.dndiscord.model.dndbeyond.Action actIn, FeatureType type) {
		...
		if (actIn.getActivation() != null) {
			...
			if (actTime != null && !actTime.isEmpty())
				action.setDuration(Integer.valueOf(actIn.getActivation().getActivationTime()));
		}
	...
	private void applySkillMods(CharacterSheet sheet) {
		...
		sheet.setAnimalHandlingMod(sheet.getWisdomMod() + (sheet.isAnimalHandlingExpertise() ? expertiseBonus
				: (sheet.isAnimalHandlingProficiency() ? profBonus : 0)));
		...
		sheet.setIntimidationMod(sheet.getCharismaMod() + (sheet.isIntimidationExpertise() ? expertiseBonus
				: (sheet.isIntimidationProficiency() ? profBonus : 0)));
		sheet.setInvestigationMod(sheet.getIntelligenceMod() + (sheet.isInvestigationExpertise() ? expertiseBonus
				: (sheet.isInvestigationProficiency() ? profBonus : 0)));
		...
		sheet.setPerformanceMod(sheet.getCharismaMod() + (sheet.isPerformanceExpertise() ? expertiseBonus
				: (sheet.isPerformanceProficiency() ? profBonus : 0)));
		...
		sheet.setSleightOfHandMod(sheet.getDexterityMod() + (sheet.isSleightOfHandExpertise() ? expertiseBonus
				: (sheet.isSleightOfHandProficiency() ? profBonus : 0)));
	...
	private void setClasses(CharacterSheet sheet, DndBeyondSheet character) {
		List<DndBeyondCharacterClass> beyondClasses = Arrays.asList(character.getClasses());
		...
		if (type != null) {
				...
				if (null != opt) {
					...
					if (myFeature != null) {
						...
						if (act != null) {
							...
							if (actType != null)
								feat.setActivationType(ActivationType.getEnum(Integer.parseInt(actType)));
							if (actTime != null)
								feat.setDuration(Integer.parseInt(actTime));
						}
			...
			case BACKGROUND:
				...
				if (background != null) {
					...
					feat.setSuggestedCharacteristicsDescription(
							background.getDefinition().getSuggestedCharacteristicsDescription());
				}
				break;
