...
public enum AttackSubtype {
	...
	static {
		valuesMap = 
			new HashMap<Integer, AttackSubtype>(AttackSubtype.values().length);
		valuesMap.put(1, UNKNOWN);
		valuesMap.put(2, NATURAL);
		valuesMap.put(3, UNARMED);
		
		stringMap = 
			new HashMap<AttackSubtype, String>(AttackSubtype.values().length);
	 ...
	 * @return an AttackSubtype containing the enumeration 
