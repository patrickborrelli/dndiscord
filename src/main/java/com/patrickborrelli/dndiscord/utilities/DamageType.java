...
public enum DamageType {
	...
	BLUDGEONING(1), PIERCING(2), SLASHING(3), NECROTIC(4), ACID(5), COLD(6), FIRE(7), LIGHTNING(8), THUNDER(9),
	POISON(10), PSYCHIC(11), RADIANT(12), FORCE(13);
	...
	static {
		valuesMap = new HashMap<Integer, DamageType>(DamageType.values().length);
		...
		stringMap = new HashMap<DamageType, String>(DamageType.values().length);
		...
		inverseMap = new HashMap<String, DamageType>(DamageType.values().length);
	 ...
	 * @return an DamageType containing the enumeration represented by the provided
	 *         Integer.
	 */
