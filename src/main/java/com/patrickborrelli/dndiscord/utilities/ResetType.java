...
public enum ResetType {
	SHORT_REST(1), LONG_REST(2), DAWN(3);
	...
	static {
		valuesMap = new HashMap<Integer, ResetType>(ResetType.values().length);
		valuesMap.put(1, SHORT_REST);
		valuesMap.put(2, LONG_REST);
		valuesMap.put(3, DAWN);

		stringMap = new HashMap<ResetType, String>(ResetType.values().length);
		stringMap.put(SHORT_REST, "Short rest");
		stringMap.put(LONG_REST, "Long rest");
		stringMap.put(DAWN, "Dawn");

		inverseMap = new HashMap<String, ResetType>(ResetType.values().length);
	 ...
	 * @return an ResetType containing the enumeration represented by the provided
	 *         Integer.
	 */
