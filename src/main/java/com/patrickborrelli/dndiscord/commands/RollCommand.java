...
/**
 * Command class to handle all types of dice rolling commands issued to the
 * DnDiscord bot.
 * 
...
public class RollCommand implements CommandExecutor {
	...
	private static final String CHARACTER_ROLL = "repeat(4d6r1k3, 6)";
	...
	private String generateRollResponse(String param, DiscordUser user, Message msg) throws MalformedEquationException {
		...
		if (isDigit(firstChar)) {
			...
			case C:
				// could either be clear or character
				if (tokens[0] != null && tokens[0].equalsIgnoreCase(CHARACTER)) {
					// process character roll:
					result.append(buildRepeatResponse(CHARACTER_ROLL));
					break;
				} else {
	...
	private void buildRollEmbed(Message msg, String contents) {
		EmbedBuilder embed = new EmbedBuilder().setTitle("Saved Rolls").setDescription(contents)
				.setAuthor("DnDiscord", "http://github.com/patrickborrelli",
						AppUtil.getInstance().getBotAvatarUrl().toString())
	...
	private String storeSavedRoll(String param, DiscordUser user) {
		...
		try {
			...
			// if we arrive here, the roll formula cannot be validated, so skip saving and
			// notify user:
	...
	private String buildRepeatResponse(String param) throws MalformedEquationException {
		...
		/**
		 * parse the command and repeatedly feed the equation to the processor the
		 * required number of times. format expected is: repeat(equation, N) where N is
